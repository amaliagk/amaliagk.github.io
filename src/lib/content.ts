export const profile = {
  name: "Amalia Gkigkolian",
  title: "Graphic Designer & Marketing Creative",
  heroLine:
    "I create strategic visual communications across branding, digital campaigns, websites, presentations, video, print, and events.",
  location: "Limassol, Cyprus",
  email: "am.gigolian@gmail.com",
  linkedin: "https://www.linkedin.com/in/amalia-gkigkolian-82b288201",
};

export const introduction = {
  paragraph:
    "I am a graphic designer with experience across corporate communications, digital marketing, campaign development, video, print, and events. Having previously worked as a Marketing Manager, I approach design not only from a visual perspective, but also through business objectives, audience needs, and brand consistency.",
  whatIDo: [
    "Brand & Visual Identity",
    "Corporate Communications",
    "Digital Marketing",
    "Events & Exhibitions",
    "Video & Motion",
    "Marketing Strategy & Coordination",
    "Web Design & Management",
  ],
};

export { caseStudies } from "./case-studies";

export const experience = [
  {
    company: "Fameline Holding Group",
    role: "Graphic Designer",
    period: "2024–Present",
    focus: [
      "Corporate and digital design",
      "Presentations",
      "Campaigns",
      "Events and exhibitions",
      "Video and motion",
      "Multi-brand marketing support",
    ],
  },
  {
    company: "Hadjiyiannis Group",
    role: "Marketing Manager",
    period: "2020–2024",
    focus: [
      "Marketing strategy",
      "Campaign planning",
      "Website and social media management",
      "Email marketing",
      "Budget and event coordination",
    ],
  },
  {
    company: "Direct Image",
    role: "Graphic Designer",
    period: "2019–2020",
    focus: [
      "Print design",
      "Artwork preparation",
      "Digital and offset print production",
    ],
  },
];

export const tools = [
  {
    group: "Design",
    items: ["Illustrator", "Photoshop", "InDesign", "Figma"],
  },
  {
    group: "Video & Motion",
    items: ["Premiere Pro", "After Effects"],
  },
  {
    group: "Web",
    items: ["WordPress/Elementor"],
  },
  {
    group: "AI",
    items: ["ChatGPT", "Claude"],
  },
];

export const education = [
  {
    program: "Graphic Design & Multimedia",
    school: "Cyprus University of Technology",
  },
  {
    program: "Interior Design",
    school: "A' Technical School of Limassol",
  },
];

export const certifications = [
  "AI Leadership for Enterprise Transformation",
  "Digital Leadership and Transformation with AI Strategy and Design Thinking",
  "Certified Digital Advertising Professional",
];
